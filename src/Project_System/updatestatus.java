...
public class updatestatus extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        estimatedtime = new javax.swing.JComboBox<>();
        ...
        estimatedtime.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "1 Hour", "2 Hours", "3 Hours", "4 Hours", "5 Hours", "6 Hours", "7 Hours", "8 Hours", "1 Day", "2 Days", "3 Days", "4 Days", "5 Days" }));

        jButton2.setText("ESTIMATE TIME");
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                .addComponent(estimatedtime, javax.swing.GroupLayout.PREFERRED_SIZE, 96, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        // ─── Called when jButton2 is clicked ───────────────────────────────────────
    String selected = (String) estimatedtime.getSelectedItem();
    if (selected == null || selected.trim().isEmpty()) {
        JOptionPane.showMessageDialog(this, "Please select an estimated time first.");
        return;
    }

    String customer     = customerframe.username.trim();
    String mechanic     = mechanicframe.mechanicname.trim();
    String estimatedCSV = "src\\estimatedtime.csv";
    java.io.File file   = new java.io.File(estimatedCSV);
    ...
    if (fileExists) {
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (cols.length >= 5
                        && cols[2].trim().equalsIgnoreCase(customer)
                        && cols[1].trim().equals(String.valueOf(targetCarSlot))) {
                    // Found existing record — update it instead of adding new
                    allLines.add(cols[0].trim() + "," + targetCarSlot + "," + customer + "," + mechanic + "," + selected.trim());
                    found = true;
                } else {
                    allLines.add(line);
    ...
    // No existing record found — add new row
    if (!found) {
        if (allLines.isEmpty()) {
            allLines.add("id,customercarid,customer,mechanic,estimatedtimetofinish");
        }
        allLines.add(nextId + "," + targetCarSlot + "," + customer + "," + mechanic + "," + selected.trim());
    }
    ...
    // Update the time textarea — replace whatever was there
    time.setText("Estimated Time: " + selected.trim());

    JOptionPane.showMessageDialog(this, "Estimated time saved!", "Saved", JOptionPane.INFORMATION_MESSAGE);
    ...
    // ─── Load saved estimated time on frame open ────────────────────────────────
public void loadEstimatedTimeFromCSV() {
    String customer = customerframe.username.trim();
    String estimatedCSV = "src\\estimatedtime.csv";
    java.io.File file = new java.io.File(estimatedCSV);
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
        ...
        while ((line = br.readLine()) != null) {
            if (firstRow) { firstRow = false; continue; } // skip header
            String[] cols = line.split(",", -1);
            // cols: id(0), customercarid(1), customer(2), mechanic(3), estimatedtimetofinish(4)
            if (cols.length >= 5
                    && cols[2].trim().equalsIgnoreCase(customer)
                    && cols[1].trim().equals(String.valueOf(targetCarSlot))) {
                String savedTime = cols[4].trim();
                String entry = "Estimated Time: " + savedTime;
                String existing = time.getText();
                if (!existing.contains(entry)) {
                    time.append((existing.isEmpty() ? "" : "\n") + entry);
                }
            }
        }
    } catch (IOException ex) {
...
// Sets servpanel text color based on the current status
    ...
    private javax.swing.JComboBox<String> estimatedtime;
